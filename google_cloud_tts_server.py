#!/usr/bin/env python3
"""
🤖 Jarvis Google Cloud TTS Server
==================================
...
def test():
    ...
    try:
        ...
        if audio_file and os.path.exists(audio_file):
            ...
            return jsonify({"error": "Google TTS test failed"}), 503
            
    except Exception as e:
        return jsonify({"error": str(e)}), 500

if __name__ == '__main__':
    print("🤖 Jarvis Google Cloud TTS Server")
    print("========================================")
    ...
    if token:
        ...
        print("❌ Service Account: Failed")
        exit(1)
    
    print("🌟 High-quality Google Cloud TTS with Neural voices")
    print("Starting server on http://localhost:5050")
    print("Endpoints:")
    print("  GET  /health  - Health check")
    print("  POST /speak   - Convert text to speech")
    print("  GET  /voices  - List available voices")
    print("  GET  /test    - Test TTS functionality")
    print("Press Ctrl+C to stop")
    
    try:
        app.run(host='0.0.0.0', port=5050, debug=False)
    except KeyboardInterrupt:
        print("\n👋 Google TTS Server stopped") 